...
app.get('/shipping/calculate', async (request, reply) => {
  ...
  // Novo cálculo de distância com a rota atualizada
  ...
  return reply.status(200).send({
    ...
    originCity: {
      ...
      state: originCity.state?.name, // Refletindo a nova estrutura
      country: originCity.country?.name,
    },
    destinationCity: {
      name: destinationCity.name,
      state: destinationCity.state?.name, // Refletindo a nova estrutura
      country: destinationCity.country?.name,
    },
  });
});
