...
app.post('/shares/files', async (request, reply) => {
  ...
  if (convertTo) {
    const inputFormat = name.split('.')[1];
    conversion = await api.conversion.createConversion(
      name,
      inputFormat,
      convertTo
    );
  ...
  const sharedFileName =
    conversion === undefined ? name : conversion.output.outputFileName;

  const originalFile =
    conversion === undefined
      ? undefined
      : { name: conversion.input.inputFileName };
