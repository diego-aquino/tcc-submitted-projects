...
app.post('/shares/files', async (request, reply) => {
  ...
  // Adaptação para a nova estrutura da API v2
  const sharedFileName =
    conversion === undefined ? name : conversion.outputFile?.name; // Acessa a nova estrutura da API v2

  const originalFile =
    conversion === undefined ? undefined : { name: conversion.inputFile?.name }; // Acessa a nova estrutura da API v2
