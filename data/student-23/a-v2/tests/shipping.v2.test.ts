...
describe('Before and after test', () => {
  ...
  test('caso 1: Quando buscamos o custo entre duas cidades existentes', async () => {
    ...
    const distanceHandlerInterceptor = distanceSchemaInterceptor
      .get(`/cities/${saoPauloId}/distances/cities/${campinaGrandeId}`)
      .respond({
        status: 200,
    ...
    expect(requests3[0].searchParams.size).toBe(0);
  ...
  test('caso 2: Verificamos se frete dentro de um mesmo estado é grátis', async () => {
    ...
    const distanceHandlerInterceptor = distanceSchemaInterceptor
      .get(`/cities/${campinaGrandeId}/distances/cities/${joaoPessoaId}`)
      .respond({
        status: 200,
        body: { kilometers: 126 },
      });
  ...
  test('caso 4: Frete grátis quando a distância é 0 (Entrega na mesma cidade)', async () => {
    ...
    const distanceHandlerInterceptor = distanceSchemaInterceptor
      .get(`/cities/${campinaGrandeId}/distances/cities/${campinaGrandeId}`)
      .respond({
        status: 200,
        body: { kilometers: 0 },
      });
