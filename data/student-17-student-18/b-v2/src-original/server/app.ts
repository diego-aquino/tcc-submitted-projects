...
app.post('/shares/files', async (request, reply) => {
  const { name, mode, convertTo, inputFileFormat } = shareFileSchema.parse(request.body);

  let conversion: Conversion | undefined;

  if (convertTo) {
    conversion = await api.conversion.createConversion(name, inputFileFormat, convertTo);
