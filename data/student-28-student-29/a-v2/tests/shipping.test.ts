...
const interceptorServer = setupServer(
  ...
  http.get(
    `${process.env.LOCATION_API_URL}/cities/:originCityId/distances/:destinationCityId`,
    () => {
    ...
    interceptorServer.use(
      http.get(
        ...
        ({ request, params }) => {
          ...
          const { originCityId } = params;
          const { destinationCityId } = params;
          console.log(`/cities/${originCityId}/distances/${destinationCityId}`); //checa a url pra ver se os ids estão sendo passados
